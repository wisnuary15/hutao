...
let handler = async (m, { conn, usedPrefix, command }) => {
	...
	if (m.sender in conn.cartoon)
		throw "*please wait, there is still a process that has not been completed*";
	let q = m.quoted ? m.quoted : m;
	let mime = (q.msg || q).mimetype || q.mediaType || "";
	if (!mime) throw `*where your image?*`;
	if (!/image\/(jpe?g|png)/.test(mime)) throw `*sorry your ${mime} not supported.*`;
	...
	try {
		Cartoon(img).then(async (response) => {
			if (response.message == "success") {
				...
				m.reply(
					"*sorry it seems the image is not detected face.*"
				);
			}
		});
	} catch {
		m.reply("*sorry progress failed.*");
	} finally {
